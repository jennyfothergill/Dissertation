...
In order to get agreement between engines and closer to the correct density value, the correction to the energy and pressure was needed (see \autoref{fig:lrc_cutoff}).
\begin{figure}[h!]
    \centering
...
%% when I calculate what this difference should be given the parameters of the simulation though, it does not make sense:

%N = 900
%kB = 8.314462618e-3 * u.kJ/(u.K*u.mol)
%T = 140 * u.K
%V = (3.998 * u.nm)**3
%N*kB*T/V
% unyt_quantity(16.3936765, 'kJ/(mol*nm**3)')

% the difference in pressures is more like 100 kJ/(mol nm^3)...
Through examination of the energy values of this first frame we also found a discrepancy in the energies reported by older version of GROMACS, see \autoref{fig:lrc_energy}.
\begin{figure}[h!]
    \centering
...
The potential energy between engines agree within error for methane \autoref{fig:methane_pe}.
\begin{figure}[h!]
    \centering
...
I'm not sure what is causing the difference in MCCCS.
\begin{figure}[h!]
    \centering
...
Use the following figures to show that automated equilibration detection works?
\begin{figure}[h!]
    \centering
